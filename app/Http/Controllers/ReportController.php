...
class ReportController extends Controller
{
    public function closing()
    {
        ...
        $salesmen = $salesmanQuery->with(['sales' => function ($q) {
            $q->where('status', 'paid');
        }])->get();
        ...
        return view('report.closing', compact(
            ...
            'cashIn', 'cashOut', 'endingBalance', 'salesmanAssessment', 'unpaidInvoices'
        ));
    }
    ...
    public function closingExportPdf()
    {
        ...
        $salesmanAssessment = $salesmen->map(function ($s) {
            ...
            $pct = $target > 0 ? min(100, round(($achievement / $target) * 100)) : 0;
            $grade = $pct >= 100 ? 'A' : ($pct >= 80 ? 'B' : ($pct >= 60 ? 'C' : 'D'));
            return ['name' => $s->name, 'achievement' => $achievement, 'target' => $target, 'percentage' => $pct, 'grade' => $grade];
        });
