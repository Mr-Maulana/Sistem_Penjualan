        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Provinsi -->
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1">Provinsi <span class="text-red-500">*</span></label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-orange-500 transition-colors">
                                <i data-lucide="globe" class="w-4 h-4"></i>
                            </div>
                            <select id="province-select" name="province" required class="w-full pl-11 pr-10 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-black text-slate-800 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all appearance-none shadow-sm">
                                <option value="">-- Pilih Provinsi --</option>
                                @foreach($provinces as $p)
                                    <option value="{{ $p }}" {{ (old('province', $supplierProvince ?? '') == $p) ? 'selected' : '' }}>{{ strtoupper($p) }}</option>
                                @endforeach
                            </select>
                        ...
                        @error('province') <div class="text-xs text-red-500 mt-1 font-medium">{{ $message }}</div> @enderror
                    </div>

                    <!-- Kota / Kabupaten -->
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1">Kota / Kabupaten <span class="text-red-500">*</span></label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-orange-500 transition-colors">
                                <i data-lucide="map-pin" class="w-4 h-4"></i>
                            </div>
                            <select id="city-select" name="city" required class="w-full pl-11 pr-10 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm font-black text-slate-800 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all appearance-none shadow-sm">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Dynamic Chained Dropdowns for Province and City
    const provinceSelect = document.getElementById('province-select');
    const citySelect = document.getElementById('city-select');
    
    // Parse grouped areas from PHP
    const areasData = @json($areasGrouped);
    const initialCity = "{{ old('city', $supplier->city ?? '') }}";
    ...
    function updateCities(selectedProvince, selectedCity = '') {
        ...
        if (selectedProvince && areasData[selectedProvince]) {
            const cities = areasData[selectedProvince];
            cities.forEach(city => {
    ...
    // Trigger initial population on edit or old input fallback
    if (provinceSelect.value) {
        updateCities(provinceSelect.value, initialCity);
    }
});
</script>
@endpush
