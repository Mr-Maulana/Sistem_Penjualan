...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 flex items-center justify-between mb-8">
    ...
    <a href="{{ route('report.closing.export.pdf', request()->query()) }}" class="bg-red-600 hover:bg-red-700 text-white text-sm font-bold px-6 py-2.5 rounded-xl flex items-center gap-2 transition-all shadow-md shadow-red-100 hover:-translate-y-0.5 active:scale-95">
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 mb-8">
    <form method="GET" action="{{ route('report.closing') }}" class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
        <div>
            <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Filter Kelompok Tim</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <i data-lucide="users" style="width:16px;height:16px;" class="text-slate-400"></i>
                </div>
                <select name="team_id" class="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all bg-slate-50/50 hover:bg-slate-50 appearance-none">
                    <option value="">-- Semua Tim --</option>
                    @foreach($teams as $t)
                        <option value="{{ $t->id }}" {{ request('team_id') == $t->id ? 'selected' : '' }}>
                            Tim {{ $t->name }} [{{ strtoupper($t->level) }} - {{ $t->area_display ?: ($t->city ?? $t->area) }}]
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Filter Perorangan (Salesman)</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <i data-lucide="user" style="width:16px;height:16px;" class="text-slate-400"></i>
                </div>
                <select name="salesman_id" class="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all bg-slate-50/50 hover:bg-slate-50 appearance-none">
                    <option value="">-- Semua Salesman --</option>
                    @foreach($salesmenList as $s)
        ...
        <div class="flex gap-3">
            <button type="submit" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold py-2.5 px-4 rounded-xl transition-all shadow-md shadow-indigo-100 flex items-center justify-center gap-2">
                <i data-lucide="filter" style="width:16px;height:16px;"></i> Filter
            </button>
            @if(request()->filled('team_id') || request()->filled('salesman_id'))
                <a href="{{ route('report.closing') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-bold py-2.5 px-4 rounded-xl transition-all flex items-center justify-center gap-2">
...
<div class="grid grid-cols-1 lg:grid-cols-1 gap-8 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden animate-slide-up">
        <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/20">
            <div class="flex items-center gap-3">
                ...
                <h3 class="font-bold text-slate-800">Assessment Manager</h3>
            </div>
            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">PERFORMA KELOMPOK TIM</div>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                @forelse($managerAssessment as $s)
                    @php
                        $grade = $s['grade'];
                        ...
                        <div class="flex items-center justify-between mb-2">
                            <div>
                                <div class="font-bold text-slate-800 text-sm flex items-center gap-2">
                                    {{ $s['name'] }}
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 uppercase">Manager</span>
                                ...
                                <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wide">
                                    Target Tim: Rp {{ number_format($s['target'], 0, ',', '.') }} | Ach: Rp {{ number_format($s['achievement'], 0, ',', '.') }}
                                </div>
                            </div>
                ...
                @empty
                    <div class="flex flex-col items-center justify-center py-4 col-span-2">
                        ...
                        <p class="text-slate-400 text-sm font-medium">Belum ada data manager</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <!-- Assessment Supervisor & Sales (Salesman Lain) -->
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden animate-slide-up mt-6">
        <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center">
                    <i data-lucide="award" style="width:18px;height:18px;"></i>
                </div>
                <h3 class="font-bold text-slate-800">Assessment Salesman Lain</h3>
            </div>
            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">PERFORMA INDIVIDU</div>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                    ...
                    <div class="group">
                        <div class="flex items-center justify-between mb-2">
                            <div>
                                <div class="font-bold text-slate-800 text-sm flex items-center gap-2">
                                    {{ $s['name'] }}
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold {{ $s['level'] === 'supervisor' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700' }} uppercase">
                                        {{ $s['level'] }}
                                    </span>
                                    @if($grade === 'A') <i data-lucide="sparkles" class="w-3.5 h-3.5 text-amber-500"></i> @endif
                                </div>
                                <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wide">
                                    @if($s['level'] === 'supervisor')
                                        Target Tim: Rp {{ number_format($s['target'], 0, ',', '.') }} | Ach Tim: Rp {{ number_format($s['achievement'], 0, ',', '.') }}
                                    @else
                                        Target: Rp {{ number_format($s['target'], 0, ',', '.') }} | Ach: Rp {{ number_format($s['achievement'], 0, ',', '.') }}
                                    @endif
                                </div>
                                @if($s['level'] === 'supervisor' && !empty($s['team_list']))
                                    <div class="text-[9px] text-indigo-500 font-semibold mt-1">
                                        Anggota: {{ $s['team_list'] }}
                                    </div>
                                @endif
                            </div>
