...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 max-w-5xl mx-auto overflow-hidden">
    ...
    <form method="POST" action="{{ isset($sale) ? route('sale.update', $sale) : route('sale.store') }}" class="p-8" id="sale-form">
        ...
        <div class="bg-slate-50 border border-slate-200 rounded-2xl mb-8 shadow-inner overflow-visible">
                ...
                <table class="w-full text-sm" id="items-table">
                    ...
                    <tbody id="items-body" class="bg-white/30 divide-y divide-slate-100">
                        ...
                        <tr class="item-row hover:bg-white/80 transition-colors">
                            <td class="px-4 py-3">
                                <div class="relative product-searchable-select w-full min-w-[260px]">
                                    <!-- Hidden input to store selected product code -->
                                    <input type="hidden" name="items[{{ $idx }}][product_code]" class="item-product item-product-code" value="{{ $it['product_code'] ?? '' }}">
                                    
                                    <!-- Select Trigger Button -->
                                    <button type="button" class="select-trigger w-full border border-slate-200 rounded-lg px-3 py-2 text-xs text-left bg-white flex items-center justify-between focus:ring-2 focus:ring-blue-500/50 outline-none">
                                        <span class="trigger-label truncate font-medium text-slate-800">
                                            @php
                                                $selProduct = $products->firstWhere('code', $it['product_code'] ?? '');
                                            @endphp
                                            {{ $selProduct ? '[' . $selProduct->code . '] - ' . $selProduct->name : '-- Pilih Produk --' }}
                                        </span>
                                        <i data-lucide="chevron-down" class="w-3.5 h-3.5 text-slate-400 shrink-0 ml-1.5"></i>
                                    </button>
                                    
                                    <!-- Dropdown Option List -->
                                    <div class="select-dropdown hidden absolute z-50 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden p-2">
                                        <!-- Search input -->
                                        <div class="relative mb-2">
                                            <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none text-slate-400">
                                                <i data-lucide="search" class="w-3.5 h-3.5"></i>
                                            </div>
                                            <input type="text" class="dropdown-search-input w-full border border-slate-200 rounded-lg pl-8 pr-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/50 bg-slate-50/50" placeholder="Cari kode atau nama produk...">
                                        </div>
                                        
                                        <!-- Options scrollable container -->
                                        <div class="options-list max-h-48 overflow-y-auto divide-y divide-slate-50">
                                            @foreach($products as $p)
                                                <button type="button" class="option-item w-full text-left px-3 py-2 text-xs hover:bg-blue-50 hover:text-blue-600 transition-colors flex flex-col gap-0.5 rounded-md" 
                                                        data-code="{{ $p->code }}" 
                                                        data-name="{{ $p->name }}"
                                                        data-default-price="{{ $p->price }}">
                                                    <span class="font-bold font-mono text-[10px] text-slate-500 option-code">[{{ $p->code }}]</span>
...
(() => {
    ...
    function bindRow(row) {
        ...
        if (productSel && priceInput) {
            productSel.addEventListener('change', async () => {
                const selectedCode = productSel.value;
        ...
        const searchSelect = row.querySelector('.product-searchable-select');
        if (searchSelect && productSel) {
            ...
            trigger.addEventListener('click', (e) => {
                e.stopPropagation();
                // Close all other active dropdowns and reset their row z-indices
                document.querySelectorAll('.item-row').forEach(r => r.classList.remove('relative', 'z-40'));
                ...
                if (!dropdown.classList.contains('hidden')) {
                    row.classList.add('relative', 'z-40');
                    searchInput.value = '';
            ...
            options.forEach(opt => {
                opt.addEventListener('click', () => {
                    ...
                    productSel.value = code;
                    trigger.querySelector('.trigger-label').textContent = `[${code}] - ${name}`;
                    dropdown.classList.add('hidden');
                    row.classList.remove('relative', 'z-40');
                    
                    // Dispatch change event to trigger price lookup and recomputation
    ...
    addBtn?.addEventListener('click', () => {
        ...
        clone.querySelectorAll('input').forEach(i => {
            ...
            else if (i.classList.contains('item-product-code')) {
                const firstOpt = tpl.querySelector('.option-item');
                if (firstOpt) {
                    i.value = firstOpt.getAttribute('data-code');
                }
            } else i.value = 0;
        });
        
        // Reset trigger label to the first item
        const firstOpt = tpl.querySelector('.option-item');
        if (firstOpt) {
            const code = firstOpt.getAttribute('data-code');
            const name = firstOpt.getAttribute('data-name');
            clone.querySelector('.trigger-label').textContent = `[${code}] - ${name}`;
        } else {
            clone.querySelector('.trigger-label').textContent = '-- Pilih Produk --';
        }
        
        const dropdown = clone.querySelector('.select-dropdown');
    ...
    // Close all searchable dropdowns and reset row z-indices when clicking outside
